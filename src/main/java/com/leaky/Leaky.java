...
public class Leaky
{
    ...
    public static void detectedItemLeak(final ItemEntity entity, final List<ItemEntity> items, final int range)
    {
        ...
        MutableComponent component = Component.literal("发现: " + items.size() + "的物品堆叠量 | 位于 ")
            ...
            .append(Component.literal("[" + entity.blockPosition().toShortString() + "]")
                .withStyle(ChatFormatting.YELLOW).withStyle(style ->
                {
                    return style.withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND,
                        "/execute in " + entity.level().dimension().location() + " run tp " + entity.getBlockX() + " " + entity.getBlockY() + " " + entity.getBlockZ()));
                }))
            .append(Component.literal(" | 维度: "))
            .append(Component.translatable(entity.level().dimension().location().toLanguageKey()))
            .append(Component.literal(" | 最近的玩家ID为 " + nearestPlayerName));
